...
void account::modify()
{
	...
	cin >> income;
	cout << "\n Введите тип учетной записи (Bring_out (B)/Insert (I)): ";
	cin >> type;
	type = toupper(type);
...
void settings_bd(const char* fname);									//функция настройки БД			
	...
	do
	{
		...
		case '1':
			if (proverka_file(fname_b)) {
				...
				cout << "\n Ошибка открытия файла для чтения и записи!Press any Key..." << endl;
				break;
			}
			cout << " Создаем базу " << fname_b << endl;
			cout << " База данных МФО готова к работе." << endl;
