...
int main() {
    ...
    case 2: 
        ...
        cout << "Integral: S2 = " << (s2 = fabs(integral(-1 - sqrt(3) / 2, -.5, n, eps, fun5, s))
            - fabs(integral(-1, -.5, n, eps, fun3, s))
            - fabs(integral(-1, -1 - sqrt(3) / 2, n, eps, fun6, s))) << endl;
        break;
    case 3: 
        cout << "Formula: S3 = " << (s3 = 2 * (M_PI / 4 - (M_PI / 3 - (sqrt(3) / 4)))) << endl;
    ...
    case 4: 
        ...
        cout << "Integral: S4 = " << (s4 = fabs(integral(0, 1 - sqrt(3) / 2, n, eps, fun9, s))
            + fabs(integral(1 - sqrt(3) / 2, 1, n, eps, fun8, s))
            + fabs(integral(0, .5, n, eps, fun10, s))
            + fabs(integral(.5, 1, n, eps, fun7, s))) << endl;
        break;
    case 5: 
        cout << "Formula: S5 = " << (s5 = 2 - (1 - M_PI / 4)) << endl;
        cout << "Integral:S5 = " << (s5 =  fabs(integral(-1, 1, n, eps, fun12, s)) 
            - fabs(integral(-.5, sqrt(3) / 2, n, eps, fun11, s))
            - fabs(integral(-1, -.5, n, eps, fun13, s)) 
            - fabs(integral(sqrt(3) / 2, 1, n, eps, fun14, s))) << endl;
    }
    cout << "\n ѕовторить-1, ¬ыход-2: ";
    cin >> i;
    if (i == 1) main();
...
/*описание вида интегрируемой функции є14 дл€ линии: x^2+(y-1)^2=1 (верхн€€ полуокружность)*/
double fun14(double x) {
    return(sqrt(1 - x * x) - 1);
}
