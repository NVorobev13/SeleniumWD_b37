...
void zag1() {
    ...
    for (i = 1; i <= 80; i++)
        cout << "-";
        cout << endl;
        //cout("|%15s|%15s|%11s|%18s|%15s|\n", "Название яхты ", "Имя владельца ", "Длина, м", "Стоимость, млн.$", "Год постройки");
...
void prosmotrbd1(const char* fname) {
    ...
    while (fread(&ship, sizeof(ship), 1, baza) > 0) {
        //cout << "\n|%15s|%15s|%11i|%18i|%15s|" << ship.title << ship.owner << ship.lenght << ship.cost << ship.yearsOld;
        cout << endl;
...
void sort_owner(const char* fname1, const char* fname2) {
    ...
    while (fread(&ship, sizeof(ship), 1, in) > 0)
        if (strncmp(ship.owner, ownerName, 20) == 0)
